...
| Before | After |
|---|---|
| Double-click `start.bat` | Start **Apache** and **MySQL** in the XAMPP Control Panel |
| `http://192.168.1.10:8080` | `https://192.168.1.10` |
| MySQL from XAMPP | MySQL from XAMPP (unchanged) |

`start.bat` will refuse to run once `.env` says `https://`, and will tell you
this, so there is no way to end up accidentally running both.
